...
struct thread_data{
    int tid, flag;  //人为给定的线程号和线程类别：0 - IO线程， 1 - 计算线程
    FILE *fin, *fout;
...
void *cal_thread_func(void *data){
    ...
    pthread_mutex_lock(&my_data->shared->mutex);
    if(my_data->IO_data[data_id].input.size < 50) pthread_cond_broadcast(&my_data->shared->cv);
    pthread_mutex_unlock(&my_data->shared->mutex);
    int wait_write;
    DeQueue(my_data->IO_data[data_id].input, wait_write);
    EnQueue(my_data->IO_data[data_id].output, wait_write);
    sleep(1);
}
...
int main() {
    ...
    for(i = 0; i < io_threads; i++){
        ...
        if(i % n_threads_one_IO) {
            ...
            data[i].fout = fopen(outputPath(../output, i, ".txt"), "w");
        }
    }
