...
extern int threshold = (int)(batchsize / (n_threads_one_IO - 1));
...
void *cal_thread_func(void *data){
    ...
    while (j < my_data->IO_data[data_id].input.rear) {
        ...
        fprintf(stderr, "tid = %d, i = %d, j = %d\n", my_data->tid, i, j);
        if (my_data->IO_data[data_id].send_singal && my_data->IO_data[data_id].not_end && (my_data->tid % n_threads_one_IO == 1) && (my_data->IO_data[data_id].input.size - i < threshold)) {
